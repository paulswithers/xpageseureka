...
var dBar = {		//debug toolbar
	...
	MAX_MESSAGES : 1000,		//maximum number of messages stored in sessionScope
		...
		synchronized(sessionScope){
			try {
				...
				if (messages.length > this.MAX_MESSAGES) {
					messages = messages.splice( this.MAX_MESSAGES, 1);
				}
				
				sessionScope.debugToolbar.logMessages = messages;
				sessionScope.get("debugToolbar2").put("logMessages", messages);
	...
	getFilteredMessages : function() {
		...
		var hide = sessionScope.debugToolbar.hideLogTypes;
		
		var r = [];
		
		for (var i=0; j = messages.length, i<j; i++) {
			if (@IsNotMember( messages[i].type, hide)) {
				r.push( messages[i]);	
			}
		}
		return r;
	...
	clearMessages : function() {
		sessionScope.get("debugToolbar").logMessages = [];
	},
		...
		try {
			...
			case "map":
		    	...
		    	if (!sessionScope.debugToolbar.showLists) {
		    		return "<span class=\"highlight\">(hidden list)</span>";
		    	}
		        ...
		        for(var item in obj) {
					...
					rows.push({
						sortVal : item,
						lbl : label,
						val : (this._hideDetails(label) ? "(skipped)" : ( (valueType=='object' || valueType=="array" || valueType=="map") ? this._dumpObject(value) : value ) )
					});
		        }
	...
	getCustomVars : function() {
		...
		try {
			...
			a.push( { key : "browser", value : context.getUserAgent().getBrowser() + " " +  context.getUserAgent().getBrowserVersion() });
			a.push( { key : "browser language", value : context.getLocale().getDisplayName() + " (" + context.getLocaleString() + ")" });
			a.push( { key : "browser timezone", value : context.getTimeZoneString() });
	...
	getTimers : function() {
		...
		try {
			...
			while(it.hasNext()) {
				...
				if (timer.completed == 0) {		//stop timer
					timer.completed = java.lang.System.currentTimeMillis();
